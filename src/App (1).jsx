...
const LANGUAGES = {
  fr: {
    ...
    radius_labels: { 15: "À côté", 35: "Tout près", 50: "Pas loin", 75: "Une vraie sortie", 100: "L'aventure" },
  ...
  en: {
    ...
    radius_labels: { 15: "Right next door", 35: "Very close", 50: "Not too far", 75: "A real outing", 100: "The adventure" },
  ...
  de: {
    ...
    radius_labels: { 15: "Gleich nebenan", 35: "Sehr nah", 50: "Nicht weit", 75: "Ein echter Ausflug", 100: "Das Abenteuer" },
  ...
  es: {
    ...
    radius_labels: { 15: "Muy cerca", 35: "Cerca", 50: "No muy lejos", 75: "Una salida real", 100: "La aventura" },
  ...
  pt: {
    ...
    radius_labels: { 15: "Logo ali", 35: "Muito perto", 50: "Não longe", 75: "Um passeio real", 100: "A aventura" },
  ...
  it: {
    ...
    radius_labels: { 15: "A due passi", 35: "Vicinissimo", 50: "Non lontano", 75: "Una vera gita", 100: "L'avventura" },
  ...
  nl: {
    ...
    radius_labels: { 15: "Vlakbij", 35: "Heel dichtbij", 50: "Niet ver", 75: "Een echte uitstap", 100: "Het avontuur" },
  ...
  ja: {
    ...
    radius_labels: { 15: "すぐそこ", 35: "とても近い", 50: "遠くない", 75: "本格的な外出", 100: "冒険" },
  ...
  ko: {
    ...
    radius_labels: { 15: "바로 옆", 35: "아주 가까이", 50: "멀지 않음", 75: "진짜 나들이", 100: "모험" },
  ...
  hi: {
    ...
    radius_labels: { 15: "बहुत पास", 35: "पास", 50: "ज़्यादा दूर नहीं", 75: "एक असली सैर", 100: "रोमांच" },
  ...
  ar: {
    ...
    radius_labels: { 15: "قريب جداً", 35: "قريب", 50: "ليس بعيداً", 75: "رحلة حقيقية", 100: "المغامرة" },
  ...
  id: {
    ...
    radius_labels: { 15: "Sangat dekat", 35: "Dekat", 50: "Tidak jauh", 75: "Perjalanan nyata", 100: "Petualangan" },
...
// ═══════════════════════════════════════════════════════════════
// 🌍 REVERSE GEOCODE — Simple et fiable (Nominatim seul)
// Utilisé pour identifier le nom d'un point précis (origine GPS)
// Pour les destinations en mer/montagne : voir findCitiesInRadius
// ═══════════════════════════════════════════════════════════════
const reverseGeocode = async (lat, lon, lang = "fr") => {
  const cacheKey = `aeria_rev_${lat.toFixed(2)}_${lon.toFixed(2)}_${lang}`;
  try {
...
// ═══════════════════════════════════════════════════════════════
// 🏙️ FIND CITIES IN RADIUS — La vraie magie
// Utilise Nominatim SEARCH avec viewbox bounded
// Stratégie multi-requêtes pour ratisser large : on cherche villes/villages/hameaux
// Marche partout : terre, îles, mer ouverte (cherche les côtes voisines)
// ═══════════════════════════════════════════════════════════════
const findCitiesInRadius = async (lat, lon, radiusKm, lang = "fr") => {
  const cacheKey = `aeria_cities_v2_${lat.toFixed(2)}_${lon.toFixed(2)}_${radiusKm}_${lang}`;
  try {
    const cached = localStorage.getItem(cacheKey);
    if (cached) return JSON.parse(cached);
  } catch {}

  // Bounding box autour du point (rayon élargi à 1.4x pour bien capter les villes en limite)
  const expanded = radiusKm * 1.4;
  const latDelta = expanded / 111;
  const lonDelta = expanded / (111 * Math.cos(lat * Math.PI / 180));
  const latMin = lat - latDelta;
  const latMax = lat + latDelta;
  const lonMin = lon - lonDelta;
  const lonMax = lon + lonDelta;

  // STRATÉGIE : Nominatim n'a pas de "donne-moi toutes les villes dans cette zone".
  // Workaround : on fait plusieurs recherches avec des termes simples mais variés,
  // bounded dans la zone géographique. Nominatim retourne les villes qui MATCHENT
  // le terme ET sont dans la viewbox. En cumulant plusieurs termes, on couvre la zone.
  //
  // Termes choisis : voyelles fréquentes dans les noms de lieux + lettres très communes
  // 5 requêtes parallèles : équilibre couverture / vitesse / rate-limit Nominatim
  const searchTerms = ["a", "e", "i", "o", "s"];
  const cities = [];
  const seenIds = new Set();
  const viewboxParam = `${lonMin},${latMax},${lonMax},${latMin}`;

  // On lance toutes les requêtes en parallèle pour la vitesse
  const promises = searchTerms.map(async (term) => {
    try {
      const url = `https://nominatim.openstreetmap.org/search?q=${term}&format=json&addressdetails=1&limit=15&accept-language=${lang}&viewbox=${viewboxParam}&bounded=1`;
  ...
  for (const results of allResults) {
    for (const place of results) {
      ...
      // Filtre 1 : dans le rayon demandé (avec tolérance 30%)
      if (dKm > radiusKm * 1.3) continue;

      // Filtre 2 : ne pas être trop près du départ (au moins 25% du rayon, sauf si radius < 20)
      const minDist = radiusKm < 20 ? 3 : radiusKm * 0.25;
      if (dKm < minDist) continue;

      // Filtre 3 : doit être une vraie ville / village / lieu habité
      const a = place.address || {};
      const cityName = a.city || a.town || a.village || a.municipality || a.hamlet || "";

      // Si pas de city directement, accepter si place_type indique un lieu habité
      const placeType = place.type || place.class || "";
      const isPopulatedPlace = ["city", "town", "village", "municipality", "hamlet", "suburb", "administrative"].includes(placeType);

      if (!cityName && !isPopulatedPlace) continue;

      // Construire le nom final
      let finalName = cityName;
      if (!finalName && isPopulatedPlace) {
        finalName = place.name || (place.display_name?.split(",")[0]?.trim()) || "";
      }
      if (!finalName) continue;

      // Région : état, région, ou pays
      const region = a.state || a.region || a.country || "";
      const fullName = finalName + (region ? ", " + region : "");

      cities.push({
        lat: pLat,
        lon: pLon,
        placeName: fullName,
        cityKey: finalName.toLowerCase(),
        dist: dKm,
        direction: getDirection(lat, lon, pLat, pLon),
        importance: place.importance || 0,
      });
    }
  }

  // Dédoublonnage par nom de ville (garde le plus important)
  const byName = new Map();
  for (const c of cities) {
    const existing = byName.get(c.cityKey);
    if (!existing || c.importance > existing.importance) {
      byName.set(c.cityKey, c);
    }
  }

  // Trier par importance × proximité (ville importante près = mieux)
  // puis par distance
  const unique = Array.from(byName.values());
  unique.sort((a, b) => {
    // Score combiné : importance prioritaire, distance secondaire
    const scoreA = a.importance * 100 - a.dist;
    const scoreB = b.importance * 100 - b.dist;
    return scoreB - scoreA;
  });

  // Limiter à 16 villes (assez pour avoir 5 bons résultats météo)
  const result = unique.slice(0, 16);
  try { localStorage.setItem(cacheKey, JSON.stringify(result)); } catch {}
  return result;
};

// Helper : direction cardinale entre 2 points
...
export default function App() {
  ...
  const runSearch=async(lat,lon,name)=>{
    ...
    try{
      // 🎯 STRATÉGIE PRINCIPALE : chercher les VRAIES VILLES dans le rayon
      let cities = await findCitiesInRadius(lat, lon, radius, lang);

      // Si pas assez de villes, élargir progressivement
      if (cities.length < 5) {
        const expandedCities = await findCitiesInRadius(lat, lon, radius * 1.5, lang);
        if (expandedCities.length > cities.length) cities = expandedCities;
      }
      if (cities.length < 3) {
        const moreCities = await findCitiesInRadius(lat, lon, radius * 2, lang);
        if (moreCities.length > cities.length) cities = moreCities;
      }

      // 🛡️ FALLBACK SÉCURITÉ : si vraiment aucune ville trouvée (cas extrême : Nominatim down,
      // ou zone très isolée), on génère des points géométriques + on les nomme via reverseGeocode
      // pour ne JAMAIS afficher d'écran vide
      if (cities.length === 0) {
        console.warn("[Aeria] Aucune ville trouvée, fallback ringPoints");
        const ringCandidates = ringPoints(lat, lon, radius, 16);
        const named = await Promise.all(ringCandidates.map(async (p) => {
          const placeName = await reverseGeocode(p.lat, p.lon, lang);
          return placeName ? {
            ...p,
            placeName,
            cityKey: placeName.toLowerCase(),
            dist: distKm(lat, lon, p.lat, p.lon),
            importance: 0,
          } : null;
        }));
        cities = named.filter(c => c !== null);
      }

      // Si après tout ça on n'a vraiment rien, on s'arrête proprement
      if (cities.length === 0) {
        clearInterval(iv);
        setError(L.city_not_found || L.error_msg);
        setLoading(false);
        return;
      }

      // Récupère météo : départ + villes candidates
      const allPoints = [{lat,lon,direction:"—"}, ...cities];
      const weather = await fetchWeatherBatch(allPoints);

      // Enrichit chaque point avec score météo
      const enriched = weather.map((w,i)=>{
        ...
        return {
          ...
          wx,
          score,
          dist: i === 0 ? 0 : (pt.dist ?? distKm(lat, lon, pt.lat, pt.lon)),
        };
      });

      // Point de départ
      const home = enriched[0];
      setOriginWx({...home, name});

      // Top 5 destinations (par score météo)
      const away = enriched.slice(1).sort((a,b)=>b.score-a.score).slice(0,5);

      // Ajoute le terrain (élévation)
      const top = await Promise.all(away.map(async(p)=>{
        const elev = await fetchElevation(p.lat, p.lon);
      ...
      console.error("[Aeria] Search error:", e);
      clearInterval(iv);
      setError(L.error_msg);
      setLoading(false);
    }
  };
  ...
  const radiusOptions=[15,35,50,75,100];
        ...
        <div style={{...S.card,marginTop:-4}} className="fade-up">
          ...
          <div style={{marginTop:20}}>
            ...
            <div style={S.radioBtns}>
              {radiusOptions.map(r=>(
                <button key={r} style={{...S.radioBtnBase,background:radius===r?"#533483":"#F9FAFB",color:radius===r?"#fff":"#374151",borderColor:radius===r?"#533483":"#E5E7EB",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,padding:"10px 6px",minHeight:54}} onClick={()=>{console.log("[Aeria] Radius clicked:",r);setRadius(r);}}>
                  <span style={{fontSize:14,fontWeight:700,lineHeight:1}}>{r} {L.radius_unit}</span>
                  <span style={{fontSize:10,fontWeight:500,opacity:radius===r?0.85:0.6,letterSpacing:"0.2px",lineHeight:1.1,textAlign:"center"}}>{L.radius_labels?.[r]||""}</span>
                </button>
              ))}
            </div>
