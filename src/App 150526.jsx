...
// ═══════════════════════════════════════════════════════════════
// 🌍 REVERSE GEOCODE — Trouve la ville la plus proche, partout
// Stratégie : Photon (komoot) en priorité, Nominatim en fallback
// Photon est basé OpenStreetMap MAIS sans le throttle agressif de Nominatim
// et surtout : il retourne les résultats TRIÉS PAR PROXIMITÉ
// ═══════════════════════════════════════════════════════════════
const reverseGeocode = async (lat, lon, lang = "fr") => {
  // CACHE pour ne pas re-questionner les mêmes coordonnées
  const cacheKey = `aeria_geo_v4_${lat.toFixed(2)}_${lon.toFixed(2)}_${lang}`;
  try {
    const cached = localStorage.getItem(cacheKey);
    if (cached) return cached === "__NULL__" ? null : cached;
  } catch {}

  // Helper interne : extraire un nom de ville propre d'un feature Photon
  const extractCityName = (props) => {
    const p = props || {};
    const osmKey = p.osm_key || "";
    const osmValue = p.osm_value || "";

    // PRIORITÉ : les lieux habités (place + city/town/village/etc.)
    const isPopulatedPlace = osmKey === "place" && 
      ["city","town","village","hamlet","municipality","suburb","district","island","islet"].includes(osmValue);

    if (!isPopulatedPlace) return null;
  ...
  // 1️⃣ PHOTON — recherche en cercle croissant
  // Photon retourne automatiquement les features triés par proximité
  // On itère sur les rayons pour augmenter les chances de trouver une vraie ville
  const photonLang = ["fr","en","de","it"].includes(lang) ? lang : "en";

  try {
    // Photon /reverse avec radius (en km) — paramètre distance_sort=true pour tri par proximité
    const url = `https://photon.komoot.io/reverse?lat=${lat}&lon=${lon}&lang=${photonLang}&limit=20&distance_sort=true`;
    const r = await fetch(url);
    const d = await r.json();

    if (d && Array.isArray(d.features) && d.features.length > 0) {
      // Pass 1 : chercher la ville HABITÉE la plus proche (priorité aux vrais lieux habités)
      for (const f of d.features) {
        const result = extractCityName(f.properties);
        if (result) {
      ...
      // Pass 2 : si aucun lieu habité, on accepte n'importe quel feature avec un nom propre
      // mais on EXCLUT les pays/régions seuls (trop génériques)
      for (const f of d.features) {
        const p = f.properties || {};
        const osmKey = p.osm_key || "";
        const name = p.name || "";

        // Exclure les pays/régions/continents seuls (trop génériques)
        if (["country","state","region","continent"].includes(p.osm_value)) continue;
        if (osmKey === "boundary") continue;

        if (name && name.length > 1) {
          const region = p.state || p.country || "";
          const result = name + (region ? ", " + region : "");
          try { localStorage.setItem(cacheKey, result); } catch {}
          return result;
        }
      }
    ...
    console.warn("[Aeria] Photon failed:", e);
  }

  // 2️⃣ FALLBACK Nominatim (au cas où Photon serait down ou silencieux)
  try {
  ...
  // 3️⃣ Échec total : cache le null pour éviter retry
...
export default function App() {
  ...
  const runSearch=async(lat,lon,name)=>{
    ...
    try{
      // 🎯 Génère 24 points en cercle (plus que 16 pour avoir plus de chances de toucher des villes)
      const candidates=ringPoints(lat,lon,radius,24);
      ...
      // 🎯 Prendre les 10 MEILLEURS candidats (par score météo) pour leur chercher un nom
      // (on en demande plus que 5 car certains seront déduplicqués ou sans nom)
      const topCandidates = enriched.slice(1).sort((a,b)=>b.score-a.score).slice(0,10);

      // Résoudre les noms via Photon en parallèle
      const enrichedWithNames = await Promise.all(topCandidates.map(async(p)=>{
        ...
        return{...p,placeName:placeName||null,terrain,hasRealName:!!placeName};
      }));

      // 🎯 DÉDUPLICATION : si plusieurs points pointent vers la même ville, on garde le meilleur score
      const seenCities = new Map();
      for (const r of enrichedWithNames) {
        if (!r.hasRealName) continue;
        const cityKey = r.placeName.toLowerCase().split(',')[0].trim();
        const existing = seenCities.get(cityKey);
        if (!existing || r.score > existing.score) {
          seenCities.set(cityKey, r);
        }
      }

      // Top 5 villes uniques par score décroissant
      const finalTop = Array.from(seenCities.values())
      ...
      clearInterval(iv);setError(L.error_msg);setLoading(false);
    }
  };
