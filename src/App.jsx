...
const reverseGeocode=async(lat,lon,lang="fr")=>{try{const r=await fetch(`https://geocoding-api.open-meteo.com/v1/reverse?latitude=${lat}&longitude=${lon}&language=${lang}&count=1`);const d=await r.json();if(d.results?.[0]){const p=d.results[0];return p.name+(p.admin1?", "+p.admin1:"");}}catch{}return null;};
